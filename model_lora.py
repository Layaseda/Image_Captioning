...
#train with 3 captions 
train_raw = df[df["split"] == "train"].sample(n=500, random_state=42)
expanded_rows = []

for _, row in train_raw.iterrows():
    for i in range(1, 4):  # Using only first 3 captions
        if pd.notna(row[f"caption_{i}"]):
            new_row = row.copy()
...
class RISCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        caption = "caption en: " + row["caption"]
...
print(f"Model + LoRA saved to: {save_dir}")
...
def display_samples(split_df, name="val"):
    print(f"\nShowing {name.upper()} samples with predicted captions...\n")
    sample_df = split_df.sample(n=min(3, len(split_df)), random_state=42).reset_index(drop=True)
    columns = [f"{name.capitalize()}_Image", "Prediction", "Reference_1", "Reference_2", "Reference_3"]
    ...
    for _, row in sample_df.iterrows():
        ...
        table.add_data(row["image"], pred_caption, *refs)

    wandb.log({f"{name}_sample_captions": table})
